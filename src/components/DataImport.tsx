    ...
    try {
      ...
      const processedData = entries.map((entry: any) => {
        ...
        return {
          ...
          "parsed.cache-used": determineCacheUsed(fulfilledBy, parsedCacheControl, xCache)
        };
      });
      ...
      if (data.log && data.log.entries) {
        ...
        const processedData = data.map(entry => {
          ...
          return {
            ...entry,
            "parsed.cache-control": parsedCacheControl,
            "parsed.cache-used": determineCacheUsed(entry["8.fulfilledBy"], parsedCacheControl, entry["4.x-cache"])
          };
        });
        onDataImport(processedData);
        onClose();
